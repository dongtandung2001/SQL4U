...
export default function ProjectPage() {
  ...
  return (
    <div className="container">
      ...
      {
        project && projectContents.map((item) => {
          return (
            <div>
              <h5 style={{color: "#89b3da"}}>{item.header}</h5>
              <p className="project-detail-p">{item.detail.replaceAll(/\\/g, "")[0].toUpperCase() + item.detail.replaceAll(/\\/g, "").slice(1)}</p>
            </div>
          )
        })
      }
